...
- **Compacted and transactional topics are out of scope**: the drain reads exactly `end - begin` records per partition, which equals the consumable count only on a plain topic. On a compacted topic (sparse offsets) or one written by transactional producers (control markers occupy offsets but are never delivered), fewer records are consumable, so the consumer idles to its `--timeout-ms` and the backup fails closed rather than writing a partial tarball — this logical path simply does not support those topics.
