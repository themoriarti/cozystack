...
PVE_SSH="${COZY_PVE_SSH:?COZY_PVE_SSH is required: the ssh destination of the Proxmox host, user@host}"
...
FIXTURE="$(cd "$(dirname "$0")/.." && pwd)/kubernetes-proxmox/tenant.yaml"
...
# The fixture is stand-agnostic through ${COZY_PVE_*} variables. GNU envsubst
# expands ${VAR} and nothing else — a ${VAR:-default} stays in the text as it
# is, set or not — so the defaults live here, the variable list is explicit so
# nothing else in the file is touched, and a fixture with a variable left in it
# is refused rather than applied.
render_tenant() {
  : "${COZY_PVE_ADDRESSES:=10.0.0.180-10.0.0.185}"
  : "${COZY_PVE_GATEWAY:=10.0.0.1}"
  : "${COZY_PVE_PREFIX:=24}"
  : "${COZY_PVE_BRIDGE:=vmbr50}"
  : "${COZY_PVE_POOL:=}"
  export COZY_PVE_ADDRESSES COZY_PVE_GATEWAY COZY_PVE_PREFIX COZY_PVE_BRIDGE COZY_PVE_POOL COZY_PROXMOX_STORAGE
  rendered=$(envsubst '$COZY_PVE_ADDRESSES $COZY_PVE_GATEWAY $COZY_PVE_PREFIX $COZY_PVE_BRIDGE $COZY_PVE_POOL $COZY_PROXMOX_STORAGE' < "$FIXTURE")
  case "$rendered" in
    *'${'*) die "tenant.yaml still carries an unexpanded variable after envsubst" ;;
  esac
  printf '%s\n' "$rendered"
}

apply_tenant() {
  render_tenant | kubectl apply -f -
}

# The tenant kubeconfig exists only after Kamaji has issued it. Fetching it is
# also the first proof that the control plane came up at all. The .conf key
# carries the address a runner outside the management cluster can reach; .svc
# is the in-cluster Service the management-side controllers use.
fetch_tenant_kubeconfig() {
  kubectl -n "$NS" get secret "kubernetes-${CLUSTER}-admin-kubeconfig" \
    -o jsonpath='{.data.super-admin\.conf}' | base64 -d > "$TENANT_KUBECONFIG"
...
# The disk on the hypervisor is the proof, so the check reads `qm config`, then
# removes what it created and reads the storage again: a deleted claim whose disk
# is still there is the leak the pre-delete hook's Step 1b exists to prevent.
...
EOF
  ...
  volid=${disk#*:}
  deadline=$(( $(date +%s) + 180 ))
  while pve "pvesm list $STORAGE" | grep -qF "$volid"; do
    [ "$(date +%s)" -lt "$deadline" ] || die "PVC deleted but $disk is still on $STORAGE: the driver did not free it"
    sleep 5
  done
  log "disk $disk is gone from $STORAGE"
}

# A replaced Machine has to converge without an operator: the departed Node must
# be removed by the cloud-controller-manager and the replacement must register
# in its place. "Without intervention" is the whole assertion, so nothing here
# touches the cluster beyond the initial delete.
check_replacement_converges() {
  ...
  die "no replacement reached Running with a nodeRef within 15m: the departed Node was not removed, or the replacement never registered"
}
...
case "${1:-}" in
  render-tenant)  render_tenant ;;
  apply-tenant)   apply_tenant ;;
  ...
  *) die "usage: $0 {render-tenant|apply-tenant|linked-clone|tenant-node|pvc-round-trip|replacement|diagnostics}" ;;
esac
