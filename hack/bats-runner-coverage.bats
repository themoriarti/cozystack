...
# The unit glob covers only top-level non-e2e files. Live suites must be named
# in packages/core/testing/Makefile; the e2e prefix only arms cluster captures.
# This checks file discovery, not whether CI invokes every make target.
#
# Parse the Makefiles without evaluating them: common-envs.mk can fetch git
# tags at parse time. The model check below pins the discovery and invocation
...
# `hack/cozytest.sh`.
#
# Keep variable arguments even when the .bats suffix is hidden in the variable.
# Quotes may belong to the argument or to the surrounding sh -c command.
brc_named_suites() {
...
@test "a make variable without a bats suffix is reported as unresolvable" {
  d=$(brc_fixture)
  brc_add_suite_file "$d" hack/e2e-apps/orphan.bats
  brc_add_runner_line "$d" '$(E2E_SUITE)'

  named=$(brc_named_suites "$d")
  [ "$named" = '$(E2E_SUITE)' ]
  out=$(brc_audit "$d")
  printf '%s\n' "$out" | grep -qF 'names $(E2E_SUITE) through a make variable'
  printf '%s\n' "$out" | grep -qF 'hack/e2e-apps/orphan.bats: run by nothing'
  rm -rf "$d"
}

@test "a quoted literal suite path is reachable" {
  d=$(brc_fixture)
  brc_add_suite_file "$d" hack/e2e-apps/wired.bats
  brc_add_runner_line "$d" '"hack/e2e-apps/wired.bats"'

  named=$(brc_named_suites "$d")
  [ "$named" = 'hack/e2e-apps/wired.bats' ]
  out=$(brc_audit "$d")
  [ -z "$out" ]
  rm -rf "$d"
}
