...
@test "a live cluster suite under an e2e directory keeps its captures" {
  # The pre-Chainsaw suites under hack/e2e-apps/ used plain filenames. This
  # fixture pins the directory-based guard so that layout retains captures if it
  # returns, even though no live suite currently uses it.
